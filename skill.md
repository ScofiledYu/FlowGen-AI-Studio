...
> 本节覆盖 2026-07-07「模型/Tab 面板隔离 + image2 主图」与 2026-07-08「Inspector 中键/Shift 框选拖入去重（全模型）」四项已验收交付。
...
#### 5.8.5 Agent 调试自检（改面板/切模型/tab 相关代码前）

1. 读本节 + §5.1–§5.7，确认改动是否触碰「勿改」列（含 **§5.8.4 拖入去重**）
2. 一次只修一类问题；**不得**顺手 refactor Omni tab / image2 主图 / 模型 IDB 键 / 拖入去重队列
...
|----|----------|----------------------|---------------------------|
| **模型 UI 面板** | Inspector 参考格/主图格、切模型、Omni/Seedance tab、底栏标签 | `NodeInspector.tsx`、`panelRefPersistence.ts`、`klingOmniTabPanelIsolation.ts`、`image2PanelRefs.ts`、`seedance20ModelSwitch.ts`、`localNodeMediaStore.ts` | `panel-partial-ref`、`panel-main-slot`、`panel-refresh-run-all`、`778990-cat-church`、`image2-panel-refs`、`seedance-panel-slot0`、`2026070802-seedance-panel`、`2026070802-kling-omni-panel`、`kling-omni-tab-isolation`、`frame-model-switch-isolation`、`panel-switch-broken-urls` |
| **生成结果** | 运行后 gp、OUTPUT/MOV 节点、缩略图条、画布大图 | `FlowEditor.tsx` run/spawn、`referencedMediaRun.ts`、`runRecovery.ts`、`spawnOutputNode.ts` | `ggggttt-panel`、`444444-panel`、`oooopppp-panel`、`model-contract`、`i2v-pipeline`、`banana-panel-clobber`、`run-error-no-stuck`、`20260709-seedance-ref-images` |
...
| **Node Details** | 弹窗参考图/视频/标签、与 gp 对齐 | `nodeDetailsPreview.ts`、`FlowEditor` previewParams | `node-details`、`model-contract`、`20260709-seedance-video1-mention`；发版加 `test:project-json-details` |
...
2. 是否触碰 §5.8 S 级模块（Omni tab / image2 主图 / IDB / 拖入去重）— 若是，只跑对应子门禁并说明
...
| Chat 发送 / tip / 轻量句 | `components/ChatPanel.tsx`（`handleAitopLlmSend`、`buildAitopTip`、`isLightweightPrompt`） | **S（§5.10）** | 同上（已并入 `test:chat-gate`） |
| 展示管线 | `utils/assistantMessageLayout.ts` | B | `test:layout` / `test:chat-pipeline` |
...
1. 改 tip / probe / 联网首轮 / 轻量句前：读本节 + `docs/LLM-CHAT-RULES-SPEC.md` §6.1  
2. **不得**为修身份串模型而给每轮请求加长禁令 tip  
3. 改完 `npm run test:chat-gate` 全绿；触碰四模式行为时加跑 `test:llm:four-mode`  
4. 向用户汇报是否触碰 §5.10
...
| `buildCanonicalInspectorPromptPatch(prompt, ctx)` | `string`, `NodeData` | `{ prompt: string }` | 运行前规范 @token |
...
### 8.2 `test:gate` 组成（`scripts/test-gate.mjs`，共 33 步）
...
> 四大域总览见 **§5.9**；日常改动画布/面板/Details/拖拽跑 `test:gate` 一步即可；发版见 §8.4。
...
| 运行后 gp / 刷新 recovery | `runRecovery.test.ts` + `referencedMediaRun.test.ts` |
...
- 调试默认关闭：`isPreloadDebugEnabled` 需 `window.__FLOWGEN_DEBUG_PRELOAD__ === true`；中键拖拽控制台日志需 `window.__FG_MIDDLE_DRAG_DEBUG === true`（内存 ring buffer 仍写入）。

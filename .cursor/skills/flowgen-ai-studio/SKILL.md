...
description: >-
  ...
  中键拖放/Inspector 锚定/MiniMap、多图生成数、分镜、Chat/LLM 注册、项目持久化或 server.js 前必读。
---
...
npm run test:chat-gate               # 日常改 Chat/LLM ~10s
...
**第 0.5 步 — 已验收勿改（2026-07-07/08/09，详见根目录 `skill.md` §5.8 + §5.9 + §5.10）：**

- **四大域冻结（§5.9）**：模型 UI 面板 / 生成结果 / 拖拽 / Node Details — 改任一项须 `npm run test:gate` 全绿（33 步，含 `all-models-three-requirements`）
...
| **Chat 身份/联网/tip（§5.10 已验收）** | `webSearchProbe.ts`、`ChatPanel` tip/轻量句 | `llm-chat-identity-contract-test.mjs` | `test:chat-gate`（已含）；发版加 `test:llm:four-mode` |
...
**必跑：** `npm test -- --run src/test/utils/backdropLabel.test.ts`（含在 `test:gate` vitest 内）
...
3. 勿把 BACKDROP 纳入 `collectSelectedRunQueue` 等运行队列
